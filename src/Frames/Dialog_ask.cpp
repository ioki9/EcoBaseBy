...
Dialog_ask::Dialog_ask(wxWindow* parent, const wxString& title,const wxString& text,const wxString& acptBtnLabel, const wxColour& acptBtnColour,
	const wxString& cancelBtnLabel,const wxPoint& pos, const wxSize& size, long style, const wxString& name) 
	: wxDialog(parent, -1, "", pos,size, style, name), m_parent{parent}
{
	...
	m_btnNO = new MaterialButton(m_mainPanel, wxID_ANY, cancelBtnLabel,true,wxDefaultPosition,wxSize(70, 35));
	...
	m_btnYES = new MaterialButton(m_mainPanel, wxID_ANY, acptBtnLabel, true, wxDefaultPosition, wxSize(90, 35));
	m_btnYES->SetButtonLineColour(*wxWHITE);
	m_btnYES->SetLabelColour(acptBtnColour);
