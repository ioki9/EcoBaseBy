	...
	const wxSize& size, long style, const wxString& name) : wxDialog(parent,id,title,pos,size,style,name) , m_parent{parent}
{
	...
	if (!m_openedAsEdit)
	{
		headerStr = wxString::FromUTF8("Добавить организацию");
		unpStr = "";
		adressStr = "";
		nameStr = "";
	...
	{
		headerStr = wxString::FromUTF8("Измененить данные организации");
		unpStr = org->unp;
	...
	MaterialButton* btn_listAdd = new MaterialButton(m_1stPage, wxID_ANY, wxString::FromUTF8("Добавить"), true, wxDefaultPosition, wxSize(80, 30));
	...
	MaterialButton* btn_listDelete = new MaterialButton(m_1stPage, wxID_ANY, wxString::FromUTF8("Удалить"), false, wxDefaultPosition, wxSize(80, 30));
	...
	MaterialButton* btn_listChange = new MaterialButton(m_1stPage, wxID_ANY, wxString::FromUTF8("Изменить"), true, wxDefaultPosition, wxSize(80, 30));
	...
	MaterialButton* btn_strgListAdd = new MaterialButton(m_1stPage, wxID_ANY, wxString::FromUTF8("Добавить"), true, wxDefaultPosition, wxSize(80, 30));
	...
	MaterialButton* btn_strgListDelete = new MaterialButton(m_1stPage, wxID_ANY, wxString::FromUTF8("Удалить"), false, wxDefaultPosition, wxSize(80, 30));
	...
	MaterialButton* btn_strgListEdit = new MaterialButton(m_1stPage, wxID_ANY, wxString::FromUTF8("Изменить"), true, wxDefaultPosition, wxSize(80, 30));
	...
	MaterialButton* btn_unitListAdd = new MaterialButton(m_1stPage, wxID_ANY, wxString::FromUTF8("Добавить"), true, wxDefaultPosition, wxSize(80, 30));
	...
	MaterialButton* btn_unitListDelete = new MaterialButton(m_1stPage, wxID_ANY, wxString::FromUTF8("Удалить"), false, wxDefaultPosition, wxSize(80, 30));
	...
	MaterialButton* btn_unitListEdit = new MaterialButton(m_1stPage, wxID_ANY, wxString::FromUTF8("Изменить"), true, wxDefaultPosition, wxSize(80, 30));
...
void Dialog_OrgAddEdit::OnCancel(wxCommandEvent& evt)
{
	...
	{
		Dialog_ask* ask = new Dialog_ask(this, wxString::FromUTF8("Выход из приложения"),
		wxString::FromUTF8("Продолжить без добавления организации невозможно. Желаете выйти?"), wxString::FromUTF8("Выйти"));
		Refresh();
		wxSafeYield(this);
...
{	
	...
	if (m_orgName->GetValue().IsEmpty())
		wxMessageBox(wxString::FromUTF8("Поле \"Название организации\" не может быть пустым."));
...
void Dialog_OrgAddEdit::OnWasteListEdit(wxCommandEvent& evt)
{
	if (m_wasteNormList->GetSelectedItemRef())
	{
		...
		MaterialButton* btnYES = new MaterialButton(main, wxID_ANY, wxString::FromUTF8("Применить"), true, wxDefaultPosition, wxSize(90, 35));
		...
		btnYES->Bind(wxEVT_COMMAND_BUTTON_CLICKED,
			[&dlg, this, codeCtrl, wasteCtrl](wxCommandEvent& evt) {
				if (codeCtrl->GetValue().IsEmpty())
					wxMessageBox(wxString::FromUTF8("Код отхода не может быть пустым"));
				else
				{
					if (m_wasteNormList->EditSelectedEntry(codeCtrl->GetValue(), wasteCtrl->GetValue()))
						dlg.Close();
					else
						wxMessageBox(wxString::FromUTF8("Ошибка изменения данных..."));
				}});
...
void Dialog_OrgAddEdit::OnWasteListDelete(wxCommandEvent& evt)
{
	if (m_wasteNormList->GetSelectedItemRef())
	{
		Dialog_ask* dlg = new Dialog_ask(this, wxString::FromUTF8("Удаление записи"),
		...
		if (dlg->GetReturnCode())
			...
			{
				wxMessageBox(wxString::FromUTF8("Возникла ошибка при удалении"));
			}
		dlg->Destroy();
	}
...
void Dialog_OrgAddEdit::OnWasteListAdd(wxCommandEvent& evt)
{
	...
	btnYES->Bind(wxEVT_COMMAND_BUTTON_CLICKED,
		[&dlg, this, codeCtrl, wasteCtrl](wxCommandEvent& evt) {
			if (codeCtrl->GetValue().IsEmpty())
				wxMessageBox(wxString::FromUTF8("Ошибка:код отхода не может быть пустым"));
			else if (!m_db.IsCodeExists(codeCtrl->GetValue()))
				wxMessageBox(wxString::FromUTF8("Ошибка:введенного кода отхода не существует."));
			else
			{
				if (m_wasteNormList->AddNewEntry(codeCtrl->GetValue(), wasteCtrl->GetValue()))
					...
					wxMessageBox(wxString::FromUTF8("Значение для данного вида отхода уже существует."));
			}});
		...
		btnYES->Bind(wxEVT_COMMAND_BUTTON_CLICKED,
			[&dlg, this, codeCtrl, dateCtrl, storageCtrl](wxCommandEvent& evt) {
				if (codeCtrl->GetValue().IsEmpty() || storageCtrl->GetValue().IsEmpty() )
					wxMessageBox(wxString::FromUTF8("Ошибка:все строки должны быть заполнены."));
				else if(!m_db.IsCodeExists(codeCtrl->GetValue()))
					wxMessageBox(wxString::FromUTF8("Ошибка:введенного кода отхода не существует."));
				else
				{
					if (m_strgList->EditSelectedEntry(codeCtrl->GetValue(), dateCtrl->GetValue(), storageCtrl->GetValue()))
						...
						wxMessageBox(wxString::FromUTF8("Ошибка изменения..."));
				}});
...
void Dialog_OrgAddEdit::OnStorageListAdd(wxCommandEvent& evt)
{
	...
	btnYES->Bind(wxEVT_COMMAND_BUTTON_CLICKED,
		[&dlg, this, codeCtrl, dateCtrl,storageCtrl](wxCommandEvent& evt) {
			if (codeCtrl->GetValue().IsEmpty() || storageCtrl->GetValue().IsEmpty())
				wxMessageBox(wxString::FromUTF8("Все строки должны быть заполнены."));
			else if(!m_db.isInitStorageViable(codeCtrl->GetValue(), dateCtrl->GetValue().Format(wxS("%Y.%m.%d") )))
				wxMessageBox(wxString::FromUTF8("Ошибка: В выбраном или предыдущих месяцах есть внесенные записи."));
			else
			{
				...
				if (m_strgList->AddNewEntry(codeCtrl->GetValue(), dateCtrl->GetValue(),storageCtrl->GetValue()))
					...
					wxMessageBox(wxString::FromUTF8("Значение уже существует."));
			}});
...
void Dialog_OrgAddEdit::OnUnitListEdit(wxCommandEvent& evt)
{
	if (int selected = m_unitList->GetNextItem(-1, wxLIST_NEXT_ALL, wxLIST_STATE_SELECTED); selected != -1)
	{
		...
		btnNO->SetButtonShadow(false);
		btnNO->SetTextFont(wxFontInfo(12).FaceName("Segoe UI").Bold());
		MaterialButton* btnYES = new MaterialButton(main, wxID_ANY,wxString::FromUTF8("Изменить"), true, wxDefaultPosition, wxSize(80, 35));
		btnYES->SetButtonLineColour(*wxWHITE);
		btnYES->SetLabelColour(gui_MainColour);
		btnYES->SetButtonShadow(false);
		btnYES->SetTextFont(wxFontInfo(12).FaceName("Segoe UI").Bold());
		...
		btnYES->Bind(wxEVT_COMMAND_BUTTON_CLICKED,
			[&dlg, this, unitCtrl,selected](wxCommandEvent& evt) {
				if (m_unitList->FindItem(-1, unitCtrl->GetValue(), false) > -1 && m_unitList->FindItem(-1, unitCtrl->GetValue(), false) != selected)
					wxMessageBox(wxString::FromUTF8("Структурное подразделение с таким названием уже существует"));
				else if (!m_openedAsEdit)
				{
...
void Dialog_OrgAddEdit::OnUnitListDelete(wxCommandEvent& evt)
{
	if (int selected = m_unitList->GetNextItem(- 1, wxLIST_NEXT_ALL, wxLIST_STATE_SELECTED); selected != -1)
	{
		...
		{
			wxSafeYield(this, false);
			Dialog_ask* dlg = new Dialog_ask(this, wxString::FromUTF8("Удаление структурного подразделения"),
			 wxString::FromUTF8("Вы уверены, что хотите удалить выбранное подразделение?"));
			Refresh();
			if (dlg->GetReturnCode())
			{
